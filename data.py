...
def load_rel_dataset(file_name, n_docs):
    file_path = join(DATA_DIR, file_name)
    rel_data = pd.read_csv(file_path, sep='\t', header=None)
    rel_data.columns = ['desc','h','t','r']
    rel_data['r'] = rel_data['r']-1
    rel_dict = {i: [] for i in range(n_docs)}
    
    for i in range(len(rel_data)):
        rel_dict[rel_data['h'].iloc[i]].append([rel_data['h'].iloc[i],rel_data['t'].iloc[i],rel_data['r'].iloc[i]])

    return rel_dict, rel_data['r'].nunique()
